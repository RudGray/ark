class OverworldMap {
  ...
  checkForFootstepCutscene() {
    ...
    if (!this.isCutscenePlaying && match) {
      // db request to get the custscene's sequence of events
      /*
      fetch('http://your-express-server.com/get-cutscene-events', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({ scenario_id, x: hero.x, y: hero.y })
      })
      .then(response => response.json())
      .then(data => {
        this.startCutscene(data);
      })
      .catch(error => {
        console.error('There was an error fetching the cutscene events:', error);
      });
      */
  ...
  // calculateWallGrid() {
  //   this.map.calculatedWalls = {};
  //   Object.keys(this.map.walls).forEach(coord =>  {
  //     const [x,y] = coord.split(",");
  //     this.map.calculatedWalls[utils.asGridCoord(x,y)] = true;
  //   })
  // }
  transformGameObjectCoordList(array) {
    return array.map(go => {
      const [x,y] = coord.split(",");
      return utils.asGridCoord(x,y);
    })
